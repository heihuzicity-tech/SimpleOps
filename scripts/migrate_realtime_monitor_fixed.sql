...
-- 2. 检查并添加session_records表的新字段
SET @sql = '';
SELECT COUNT(*) INTO @col_exists 
FROM information_schema.columns 
WHERE table_schema = 'bastion' AND table_name = 'session_records' AND column_name = 'last_activity';

IF @col_exists = 0 THEN
    ALTER TABLE session_records ADD COLUMN last_activity TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '最后活动时间';
END IF;

SELECT COUNT(*) INTO @col_exists 
FROM information_schema.columns 
WHERE table_schema = 'bastion' AND table_name = 'session_records' AND column_name = 'monitor_count';

IF @col_exists = 0 THEN
    ALTER TABLE session_records ADD COLUMN monitor_count INT DEFAULT 0 COMMENT '监控次数';
END IF;

SELECT COUNT(*) INTO @col_exists 
FROM information_schema.columns 
WHERE table_schema = 'bastion' AND table_name = 'session_records' AND column_name = 'is_terminated';

IF @col_exists = 0 THEN
    ALTER TABLE session_records ADD COLUMN is_terminated BOOLEAN DEFAULT FALSE COMMENT '是否被终止';
END IF;

SELECT COUNT(*) INTO @col_exists 
FROM information_schema.columns 
WHERE table_schema = 'bastion' AND table_name = 'session_records' AND column_name = 'termination_reason';

IF @col_exists = 0 THEN
    ALTER TABLE session_records ADD COLUMN termination_reason TEXT COMMENT '终止原因';
END IF;

SELECT COUNT(*) INTO @col_exists 
FROM information_schema.columns 
WHERE table_schema = 'bastion' AND table_name = 'session_records' AND column_name = 'terminated_by';

IF @col_exists = 0 THEN
    ALTER TABLE session_records ADD COLUMN terminated_by BIGINT COMMENT '终止者用户ID';
END IF;

SELECT COUNT(*) INTO @col_exists 
FROM information_schema.columns 
WHERE table_schema = 'bastion' AND table_name = 'session_records' AND column_name = 'terminated_at';

IF @col_exists = 0 THEN
    ALTER TABLE session_records ADD COLUMN terminated_at TIMESTAMP NULL COMMENT '终止时间';
END IF;
