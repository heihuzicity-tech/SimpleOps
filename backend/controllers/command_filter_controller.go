...
// GetCommandFilterLogs 获取过滤日志列表
// @Summary      获取过滤日志列表
// @Description  获取命令过滤日志，支持多种查询条件
// @Tags         命令过滤
// @Accept       json
// @Produce      json
// @Param        page       query    int     false  "页码，默认1"           minimum(1)
...
// @Param        filter_id  query    int     false  "过滤规则ID"
// @Param        action     query    string  false  "动作类型"
...
// @Failure      400        {object} utils.ErrorResponse  "参数错误"
// @Failure      500        {object} utils.ErrorResponse  "服务器内部错误"
// @Router       /api/command-filter/logs [get]
// @Security     BearerAuth
func (cf *CommandFilterController) GetCommandFilterLogs(c *gin.Context) {
	// 解析请求参数
	var req models.CommandFilterLogListRequest
	...
	if assetID, err := strconv.ParseUint(c.Query("asset_id"), 10, 32); err == nil {
		req.AssetID = uint(assetID)
	}
	if filterID, err := strconv.ParseUint(c.Query("filter_id"), 10, 32); err == nil {
		req.FilterID = uint(filterID)
	}
	req.Action = c.Query("action")
	...
	// 调用服务
	result, err := cf.commandMatcherService.GetFilterLogs(&req)
	if err != nil {
		utils.RespondWithInternalError(c, err.Error())
		return
	}
...
// GetCommandFilterLogStats 获取过滤日志统计
// @Summary      获取过滤日志统计
// @Description  获取命令过滤日志的统计信息
// @Tags         命令过滤
// @Accept       json
// @Produce      json
// @Param        start_time query    string  false  "开始时间，格式：2006-01-02 15:04:05"
// @Param        end_time   query    string  false  "结束时间，格式：2006-01-02 15:04:05"
// @Success      200        {object} models.CommandFilterLogStatsResponse  "获取成功"
// @Failure      400        {object} utils.ErrorResponse                   "参数错误"
// @Failure      500        {object} utils.ErrorResponse                   "服务器内部错误"
// @Router       /api/command-filter/logs/stats [get]
// @Security     BearerAuth
func (cf *CommandFilterController) GetCommandFilterLogStats(c *gin.Context) {
	var req models.CommandFilterLogStatsRequest
	...
	// 调用服务
	result, err := cf.commandMatcherService.GetLogStatistics(&req)
	if err != nil {
		utils.RespondWithInternalError(c, err.Error())
		return
	}
